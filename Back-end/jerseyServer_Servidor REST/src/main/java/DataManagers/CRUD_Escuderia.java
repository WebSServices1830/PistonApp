...
public class CRUD_Escuderia {
	...
	MongoCollection<co.edu.javeriana.ws.rest.clases.Escuderia> collection = database.getCollection("escuderias",
			Escuderia.class);
	...
	public List<Escuderia> escuderia_getAllBySearchParameter(String parametroBusquedaNombre) {
		...
		collection.find(regex("nombre", "(?i)(^.*" + parametroBusquedaNombre + ".*$)")).forEach(saveBlock);

		return escuderias;
	}

	public void escuderia_update(Escuderia escuderia, String id_escuderia)
			throws MongoWriteException, MongoWriteConcernException, MongoException {
		collection.updateOne(eq("id_str", id_escuderia),
				combine(set("nombre", escuderia.getNombre()), set("lugarBase", escuderia.getLugarBase()),
						set("jefeEquipo", escuderia.getJefeEquipo()), set("jefeTecnico", escuderia.getJefeTecnico()),
						...
						set("fotoEscudo_ref", escuderia.getFotoEscudo_ref()), set("autos", escuderia.getAutos()),
